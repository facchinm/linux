...
static int pm4125_connect_port(struct pm4125_sdw_priv *sdw_priv, u8 port_idx, u8 ch_id, bool enable)
{
	...
	if (!port_idx || port_idx > PM4125_MAX_SWR_PORTS) /* Invalid port index */
		return -EINVAL;
	...
	if (mstr_port_num >= SDW_MAX_PORTS)
		return -EINVAL;
...
static int pm4125_get_swr_port(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	portidx = sdw_priv->ch_info[ch_idx].port_num;
	if (portidx >= PM4125_MAX_SWR_CH_IDS)
		return -EINVAL;
...
const struct snd_soc_component_driver soc_codec_dev_pm4125 = {
	.name = "pm4125_codec",
